...
// FormatMetadata returns metdata about the endpoint in the format
// / <global ID Prefix>:<cluster name>:<node name>:<endpoint ID> as a string.
func (e *Endpoint) FormatMetadata() string {
	var metadata []string
	metadata = append(metadata, CiliumGlobalIdPrefix)
	metadata = append(metadata, ipcache.AddressSpace)
	nodeIdentity, _ := node.GetLocalNode()
	metadata = append(metadata, nodeIdentity.Name)
	metadata = append(metadata, strconv.Itoa(int(e.ID)))
