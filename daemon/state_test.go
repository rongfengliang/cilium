...
func (ds *DaemonSuite) generateEPs(baseDir string, epsWanted []*e.Endpoint, epsMap map[uint16]*e.Endpoint) ([]string, error) {
	...
	ds.OnUpdateNetworkPolicy = func(endpoint_policy_name string, id identity.NumericIdentity, policy *policy.L4Policy,
		labelsMap identity.IdentityCache, deniedIngressIdentities, deniedEgressIdentities map[identity.NumericIdentity]bool) error {
		return nil
	}

	ds.OnRemoveNetworkPolicy = func(endpoint_policy_name string) {}
