...
func (d *Daemon) UpdateNetworkPolicy(endpoint_policy_name string, id identity.NumericIdentity, policy *policy.L4Policy,
	...
	return d.l7Proxy.UpdateNetworkPolicy(endpoint_policy_name, id, policy, labelsMap, deniedIngressIdentities, deniedEgressIdentities)
}

// RemoveNetworkPolicy removes a network policy from the set published to
// L7 proxies.
func (d *Daemon) RemoveNetworkPolicy(endpoint_policy_name string) {
	if d.l7Proxy == nil {
		return
	}
	d.l7Proxy.RemoveNetworkPolicy(endpoint_policy_name)
}
