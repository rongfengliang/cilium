...
Config::Config(const Json::Object &config, Server::Configuration::FactoryContext& context)
    : Config(ApiConfigSource(*config.getObject("api_config_source")), config.getString("listener_id"), config.getString("access_log_path"), context) {}

Config::Config(const ::cilium::L7Policy &config, Server::Configuration::FactoryContext& context)
    : Config(config.api_config_source(), config.listener_id(), config.access_log_path(), context) {}
